package textEditBar
{
	...
	public class FileIOHelper
	{
		...
		//gilad: import from html to editor
		static public function importHTML(s:String,changeContent:Function=null):TextFlow{
			
			var textImporter:ITextImporter = TextConverter.getImporter(TextConverter.TEXT_FIELD_HTML_FORMAT);
			var newFlow:TextFlow = textImporter.importToFlow(s);
			reportImportErrors(textImporter.errors);
			// no TextFlow found - Flow will display an empty TextFlow by design 
			// - alternative is to do some kind of error string
			//gilad: this crap is because adobe code adds an extra \n in the import
			var clist:Array =newFlow.mxmlChildren;
			clist.shift();
			newFlow.mxmlChildren = clist;
		...
		static private function reportImportErrors(errors:Vector.<String>):void
		{
			if (errors)
			{
				...
				//PopUpManager.addPopUp(dlg, TheWhiteRabitDeskTop.thisInstance, true);
		...
		static public function export(activeFlow:TextFlow, format:String) : String
		{
			//CONFIG::debug
			{	
			...
			//gilad: this is a specific GUI implementation that i didnt like
